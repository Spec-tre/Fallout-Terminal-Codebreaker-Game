...
def display_terminal_interface(): #Fallout inspired intro
    ...
    print("=" * 40)
    slow_print("""
Welcome to Vault-Tec's Codebreaker Advanced Terminal.
You have 5 attempts to guess the correct passcode.
    """)
    print("=" * 40)
    slow_print("VALID DIGITS: 1 2 3 4 5 6 7 8 9")
    print("=" * 40)

def generate_code():
    
    return [str(random.randint(1, 9)) for _ in range(5)]

def guess_code():
    
    while True:
        guess = input("ENTER CODE (space-separated digits): ").split()
        if len(guess) != 5 or any(num not in "123456789" for num in guess):
            slow_print("INVALID ENTRY. USE FOUR DIGITS BETWEEN 1-9.")
        else:
            return guess
